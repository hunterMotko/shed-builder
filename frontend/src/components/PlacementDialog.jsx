...
export const PlacementDialog = ({
  ...
  useEffect(() => {
    if (usePreset && selectedPreset < PRESETS_BY_TYPE[placementType].length) {
      const preset = PRESETS_BY_TYPE[placementType][selectedPreset];
      setWidth(preset.width);
      setHeight(preset.height);
    }
  }, [selectedPreset, placementType, usePreset]);

  // Update preset selection when type changes
  useEffect(() => {
    setSelectedPreset(0);
  }, [placementType]);
  ...
  const handleAddPlacement = () => {
    ...
    // Warn about overlaps but allow user to proceed
    const newPlacement = {
      id: uuidv4(),
      type: placementType,
      wall,
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-800">
            Add {placementType === 'door' ? 'Door' : 'Window'}
          </h2>
          <p className="text-gray-600 text-sm mt-1">
            Placing on {wallName} wall
          </p>
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="type"
                value="door"
                checked={placementType === 'door'}
                onChange={(e) => setPlacementType(e.target.value)}
                className="w-4 h-4"
              />
              <span className="text-sm text-gray-700">Door</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
